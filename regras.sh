...
#sudo ln -s /home/aluno/firewall.sh /etc/init.d
#update-rc.d firewall.sh defaults 3
...
iptables -A FORWARD -p tcp -s $SSH2 -d $DNS2 -m multiport --dports 25,110,143,587,993,995 -j ACCEPT
...
###### 7.0 - EMAIL
iptables -A FORWARD -p tcp -s $LOCAL -d $EMAIL --dport 57000 -j ACCEPT
iptables -A FORWARD -p tcp -s $EMAIL -d $LOCAL --sport 22 -j ACCEPT
iptables -t nat -A PREROUTING -p tcp -s $LOCAL -d $SSH --dport 57000 -j DNAT --to $EMAIL:22

###### 8.0 - EMAIL
iptables -A FORWARD -p tcp -s $LOCAL -d $SAMBA --dport 58000 -j ACCEPT
iptables -A FORWARD -p tcp -s $SAMBA -d $LOCAL --sport 22 -j ACCEPT
iptables -t nat -A PREROUTING -p tcp -s $LOCAL -d $SSH --dport 58000 -j DNAT --to $SAMBA:22
